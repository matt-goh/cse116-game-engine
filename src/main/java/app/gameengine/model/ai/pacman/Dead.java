...
public class Dead extends Decision {
    ...
    /**
     * Constructs a Dead decision for the given ghost.
     *
    ...
    /**
     * Returns true if the ghost is in Dead state.
     *
     * @param dt    the time delta
     * @param level the current level
     * @return true if ghost state is "Dead"
     */
    @Override
    ...
    /**
     * Performs dead behavior - returns to ghost house.
     * Uses getHomeTarget() to find the way home.
     *
     * @param dt    the time delta
     * @param level the current level
     */
    ...
    public void doAction(double dt, Level level) {
        if (PacmanUtils.canAct(ghost, dt, lastTile)) {
            ...
            Vector2D target = PacmanUtils.getHomeTarget(pacmanLevel);
